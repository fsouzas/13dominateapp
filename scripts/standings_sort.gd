...
func sort_standing():
	for i in range(3, UniversalDict.armory_data.size()):
		...
		if UniversalDict.armory_data.values()[i]["Rank"] == "Dropped":
			temp_standing_less.get_child(1).get_child(0).text = "Dropped"
		else:
			temp_standing_less.get_child(1).get_child(0).text = str(i + 1)
		temp_standing_less.get_child(1).get_child(1).text = UniversalDict.armory_data.values()[i]["Name"].to_upper()
		temp_standing_less.get_child(1).get_child(2).text = UniversalDict.armory_data.values()[i]["Wins"].to_upper()
